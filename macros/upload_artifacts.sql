...
{% endset %}
{% set put_query %}
    PUT file://{{ filename }} @{{ database }}.dev_ccarroll.DBT_LOAD auto_compress=true;
{% endset %}
{% set copy_query %}
    BEGIN;
    COPY INTO {{ database }}.dev_ccarroll.{{ table | upper }} FROM
        @{{ database }}.dev_ccarroll.DBT_LOAD
        file_format=(type='JSON')
        on_error='skip_file';
    COMMIT;
{% endset %}
{% set remove_query %}
    REMOVE @{{ database }}.dev_ccarroll.DBT_LOAD pattern='.*.json.gz';
{% endset %}
